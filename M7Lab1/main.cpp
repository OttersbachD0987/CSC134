...
public:
    // Apply the Binary Tree algorithm to generate a maze
    static void on(Grid& grid) {
        std::vector<std::pair<int, int>> openSet = std::vector<std::pair<int, int>>();
        std::vector<bool> closedSet = std::vector<bool>();
        std::vector<int> uses = std::vector<int>();
        int rowss = grid.getRows();
        int colss = grid.getCols();
        for (int r = 0; r < grid.getRows(); ++r) {
            for (int c = 0; c < grid.getCols(); ++c) {
                //openSet.push_back(std::pair<int, int>(r, c));
        ...
        size_t iterat = 10000;
        size_t choice = 0;
        std::vector<Direction> neighbors = std::vector<Direction>();
        
        openSet.push_back(std::pair<int, int>(0, 0));
        while (openSet.size() > 0 && --iterat > 0) {
            choice = rand() % openSet.size();
            neighbors.clear();
            int row = openSet[choice].first;
            int col = openSet[choice].second;
            if (row > 0 && !closedSet[(row - 1) * rowss + (col)]) {
                neighbors.push_back(Direction::WEST);
            }
            if (row < rowss - 1 && !closedSet[(row + 1) * rowss + (col)]) {
                neighbors.push_back(Direction::EAST);
            }
            if (col > 0 && !closedSet[(row) * rowss + (col - 1)]) {
                neighbors.push_back(Direction::NORTH);
            }
            if (col < colss - 1 && !closedSet[(row) * rowss + (col + 1)]) {
            ...
            if (neighbors.size() <= 0) {
                ...
                grid.linkCells(row, col, opt);
                switch (opt) {
                    case Direction::NORTH:
                        if (!closedSet[(row) * rowss + (col - 1)]) {
                            openSet.push_back(std::pair<int, int>(row, col - 1));
                        }
                        break;
                    case Direction::EAST:
                        if (!closedSet[(row + 1) * rowss + (col)]) {
                            openSet.push_back(std::pair<int, int>(row + 1, col));
                        }
                        break;
...
void print_help(const char* program_name) {
    std::cout << "MazeBuilder - A maze generation tool using the Binary Tree algorithm\n\n";
    std::cout << "Usage: " << program_name << " [OPTIONS] [rows] [columns]\n\n";
    std::cout << "Arguments:\n";
    std::cout << "  rows          Number of rows in the maze (default: 10)\n";
    std::cout << "  columns       Number of columns in the maze (default: 10)\n\n";
    std::cout << "Options:\n";
    std::cout << "  -h, --help    Show this help message and exit\n\n";
    std::cout << "Examples:\n";
    std::cout << "  " << program_name << "                    Generate a 10x10 maze\n";
    std::cout << "  " << program_name << " 20                 Generate a 20x20 maze\n";
    std::cout << "  " << program_name << " 15 25              Generate a 15x25 maze\n";
    std::cout << "  " << program_name << " --help             Show this help message\n\n";
    std::cout << "Algorithm:\n";
    std::cout << "  Binary Tree: For each cell, randomly carve a passage either north or east.\n";
    std::cout << "  Creates a perfect maze with exactly one path between any two points.\n";
    std::cout << "  Has a bias toward the northeast corner.\n";
}
